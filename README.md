...
出于**个人真实需要**而自发研发的 AI 应用——不是对标谁、复刻谁，而是解决一个问题：**普通人缺一个把自己的时间、目标、人际、财务放在一处统一记录、统一复盘的经营系统**。
...
- **五域识别（v2 · AI-First）**：一次 GLM 调用联合抽取 日程/待办/收支/心情/饮食（+人物联动），置信度分流——高置信直接落库、低置信转待确认；输出经 zod 严格契约校验，失败自动带错误清单重问一次
