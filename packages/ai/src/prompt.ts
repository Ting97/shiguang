...
- **时态定日期**：过去式话术（"开了/完成了/刚…"）且钟点在今天已过 → 日期=今天；未来词（明天/下周/待会儿）→ 未来日期；凌晨（0-5点）补记白天的 → 昨天。
...
- true 时 start/end 必填（"YYYY-MM-DDTHH:MM" 北京时间，按「当前时间」推算实际日期）：显式起止按话术；大概时段给合理区间；刚发生→end=当前时间、start=end-durationMin；跨天 end 给次日；补记昨天填昨天日期
- 量词不是钟点（"一点点/两杯/有点"）
