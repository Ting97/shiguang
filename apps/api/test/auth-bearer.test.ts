...
import { generateSessionToken } from "../src/server/identity/auth-crypto.js";
...
test("CORS：未带凭证的未知 origin 一律拒绝（不回显）", () => {
  for (const h of [
    ...
    { Origin: "https://evil.example", Authorization: "Basic xxx" } as Record<string, string>,
    { Origin: "null" } as Record<string, string>,
  ]) {
