    ...
    <div className="glass mt-4 rounded-2xl p-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-sm font-semibold text-slate-300">
          ✨ AI 年报
          <span className="ml-2 text-[11px] font-normal text-slate-500">{year} 年</span>
        </h2>
        <button
          onClick={generate}
      ...
      {busy && <p className="mt-3 animate-pulse text-xs text-purple-300/80">正在通读本年的时间/待办/收支/人际…</p>}
      {!busy && !review && !hasRecords && <p className="mt-3 text-xs text-slate-600">本周还没有记录</p>}
      {!busy && !review && hasRecords && (
        <p className="mt-3 text-xs text-slate-600">让 AI 通读本周的时间投入/待办/收支/人际，总结这一年</p>
      )}
      {!busy && review && (
