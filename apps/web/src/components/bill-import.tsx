        ...
        {result ? (
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full rounded-xl border border-dashed border-slate-600 bg-slate-950/40 px-4 py-6 text-center text-sm text-slate-400 transition hover:border-sky-500/60 hover:text-sky-300"
            >
              {fileName ? (
                <>📄 {fileName}<span className="ml-2 text-xs text-slate-500">点击更换</span></>
              ) : (
                <>点击选择账单 CSV 文件<span className="mt-1 block text-xs text-slate-600">支付宝 / 微信官方导出，编码自动识别</span></>
              )}
            </button>
            <details>
              <summary className="cursor-pointer text-xs text-slate-500 hover:text-slate-400">或粘贴账单文本</summary>
              <textarea
                value={text}
                onChange={(e) => {
                ...
                className="mt-2 w-full resize-none rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-xs outline-none focus:border-sky-500"
              />
            </details>
            ...
            <div className="flex justify-end gap-2">
              <button onClick={onClose} className="rounded-lg px-4 py-1.5 text-xs text-slate-400 hover:bg-slate-700">取消</button>
              <button
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-950/50 px-3 py-2.5">
              <span
              ...
              <span className="text-xs text-slate-400">
                {preview.total} 笔 · 可导入 <b className="text-sky-300">{preview.importable}</b>
                {dupTotal > 0 && <span className="text-amber-300"> · 重复跳过 {dupTotal}</span>}
                {preview.skipped > 0 && <span className="text-slate-500"> · 不可导入 {preview.skipped}</span>}
              </span>
            </div>

            <div className="grid grid-cols-2 gap-2 text-center text-xs">
              <div className="rounded-lg bg-slate-950/50 px-3 py-2">
                <p className="text-slate-500">可导入支出</p>
                <p className="mt-0.5 font-semibold tabular-nums text-rose-300">{fmtMoney(preview.outCents)}</p>
              </div>
              <div className="rounded-lg bg-slate-950/50 px-3 py-2">
                <p className="text-slate-500">可导入收入</p>
                <p className="mt-0.5 font-semibold tabular-nums text-emerald-300">{fmtMoney(preview.inCents)}</p>
              </div>
            </div>
            ...
            {preview.sample && preview.sample.length > 0 && (
              <ul className="max-h-36 space-y-1 overflow-y-auto rounded-lg border border-slate-800 bg-slate-950/40 p-2 text-[11px] text-slate-400">
                {preview.sample.map((s, i) => (
                  <li key={i} className="flex items-center gap-2">
                    <span className={`rounded px-1 text-[9px] ${s.direction === "out" ? "bg-rose-500/15 text-rose-300" : "bg-emerald-500/15 text-emerald-300"}`}>
                      {s.direction === "out" ? "支" : "收"}
                    </span>
            ...
            <div className="flex flex-wrap items-center gap-1.5 text-xs">
              <span className="text-slate-400">记入账户：</span>
              {accounts.map((a) => (
                <button
                  key={a.id}
                  onClick={() => setAccountId(accountId === a.id ? "" : a.id)}
                  className={`rounded-full px-3 py-1 text-[11px] transition ${
                    accountId === a.id ? "bg-sky-600 text-white" : "bg-slate-800 text-slate-300 hover:bg-slate-700"
                  }`}
                >
                  {a.icon} {a.name}
                </button>
              ))}
              <span className="text-[10px] text-slate-600">（不选则不记账户）</span>
            </div>
            ...
            <div className="flex justify-between gap-2">
              <button
                ...
                className="rounded-lg px-4 py-1.5 text-xs text-slate-400 hover:bg-slate-700"
              >
                ← 重新选择
              </button>
              <button
